"""
Rich dynamic demo scenario data for live demonstrations, end-to-end testing, and UI validation.
...
def seed_demo_scenario(session: Optional[Session] = None, reset: bool = False) -> Dict[str, Any]:
    ...
    def _execute(s: Session) -> Dict[str, Any]:
        ...
        # -------------------------------------------------------------
        # 2. Sensor Telemetry Readings & Alarm Triggers
        # -------------------------------------------------------------
        sensors = s.query(Sensor).all()
        for snr in sensors:
            # Generate a time-series burst of 6 readings over the last 3 hours
            for step in range(6):
                reading_time = now - timedelta(minutes=30 * (5 - step))
                if snr.type == SensorType.WATER_LEVEL:
                    # Ascending flood curve exceeding warning/critical
        ...
        if shelter1:
            shelter1.current_occupancy = 680  # Evacuees checked in
            s.add(shelter1)

        shelter2 = s.query(Shelter).filter(Shelter.id == "shl-gangtok-community-002").first()
        if shelter2:
            shelter2.current_occupancy = 210  # Stranded commuters sheltered
            s.add(shelter2)

        # -------------------------------------------------------------
        # 6. Responder Mobilization & Tactical Dispatch
        # -------------------------------------------------------------
        if commander_ndrf:
        ...
        if boat_res:
            boat_res.quantity = 12.0  # 6 boats deployed
            boat_res.allocated_incident_id = "inc-demo-flood-anilnagar-002"
            s.add(boat_res)

        # Audit log for demo scenario injection
        audit = AuditLog(
...
if __name__ == "__main__":
    ...
    logger.info("Injecting dynamic disaster demo scenario...")
    res = seed_demo_scenario()
    logger.info(f"Demo scenario injected successfully: {res}")
