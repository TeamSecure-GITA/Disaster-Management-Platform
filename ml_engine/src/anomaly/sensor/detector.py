"""Sensor anomaly detector implementation."""
...
class SensorAnomalyDetector(BaseAnomalyDetector):
    """Unsupervised Isolation Forest detector for sensor anomalies."""

    def __init__(self, contamination: float = 0.05) -> None:
        super().__init__(contamination=contamination)
        self.estimator = IsolationForest(contamination=contamination, random_state=42)
        self.extractor = SensorAnomalyFeatures()
        self.preprocessor = SensorAnomalyPreprocessor()

    def fit(self, X: Any) -> SensorAnomalyDetector:
