"""Weather anomaly detector implementation."""
...
class WeatherAnomalyDetector(BaseAnomalyDetector):
    """Unsupervised Isolation Forest detector for weather anomalies."""

    def __init__(self, contamination: float = 0.05) -> None:
        super().__init__(contamination=contamination)
        self.estimator = IsolationForest(contamination=contamination, random_state=42)
        self.extractor = WeatherAnomalyFeatures()
        self.preprocessor = WeatherAnomalyPreprocessor()

    def fit(self, X: Any) -> WeatherAnomalyDetector:
