"""Infrastructure anomaly detector implementation."""
...
class InfrastructureAnomalyDetector(BaseAnomalyDetector):
    """Unsupervised Isolation Forest detector for infrastructure anomalies."""

    def __init__(self, contamination: float = 0.05) -> None:
        super().__init__(contamination=contamination)
        self.estimator = IsolationForest(contamination=contamination, random_state=42)
        self.extractor = InfrastructureAnomalyFeatures()
        self.preprocessor = InfrastructureAnomalyPreprocessor()

    def fit(self, X: Any) -> InfrastructureAnomalyDetector:
